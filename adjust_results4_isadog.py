...
def adjust_results4_isadog(results_dic, dogfile):
    ...
    with open (dogfile,'r') as f:
        for x in f.readlines():
            x=x.strip()
            if x in dognameslist:
                print(f'{x} already exists in list')
                pass
            else:
    ...
    for key,value in results_dic.items():
        ...
        elif value[0] not in dognameslist:
            results_dic[key].append(0)
        
        #check that classifier label is a dog
        if value[1] in dognameslist:
            results_dic[key].append(1)
        elif value[1] not in dognameslist:
            results_dic[key].append(0)
